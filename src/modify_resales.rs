...
impl Marketplace {
    ...
    pub fn revoke_resale(
        ...
        require!(self.owned_tickets_per_account.get(&env::predecessor_account_id()).unwrap().unwrap().iter().map(|ticket| ticket.public_key.clone()).collect::<Vec<PublicKey>>().contains(&public_key), "Must own the access key being de-list!");
