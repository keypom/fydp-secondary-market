...
impl Marketplace {
    pub(crate) fn create_event_details(
        ...
        let mut ticket_info: UnorderedMap<DropId, TicketInfo> =
            UnorderedMap::new(StorageKeys::TicketInfoPerEventInner { identifier_hash });
    ...
    pub(crate) fn assert_event_active(&self, event_id: &EventID) {
        ...
        require!(
            self.event_by_id.get(event_id).unwrap().status != Status::Inactive,
            "Event is not active"
        );
    }
    ...
    pub(crate) fn assert_resales_active(&self, event_id: &EventID) {
        let status = self
            ...
            .status;
        require!(
            status != Status::NoResales && status != Status::Inactive,
            "Event resale market is not active"
        );
    }
    ...
    pub(crate) fn price_check(&self, current_price: U128, drop_id: DropId) {
        ...
        let event_id = self
            .event_by_drop_id
            .get(&drop_id)
            .expect("No event found for drop, cannot set max price");
        let event = self
            .event_by_id
            .get(&event_id)
            .expect("No event found for event ID, cannot set max price");
        let base_price = event
            .ticket_info
            .get(&drop_id)
            .expect("No base price found for drop, cannot set max price")
            .price;

        let calculated_max_price =
            (u128::from(base_price.clone()) * u128::from(event.max_markup)) / (100 as u128);
        ...
        // Evaluate
        near_sdk::log!(
            "Received Price: {}, Max Price: {}",
            u128::from(current_price),
            adjusted_max_price.0
        );
