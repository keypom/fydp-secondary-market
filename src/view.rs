...
impl Marketplace {
    pub fn get_max_resale_for_drop(&self, drop_id: DropId) -> U128 {
        ...
        let event = self
            .event_by_id
            .get(&event_id)
            .expect("No event found for event");
        let base_price = event
            .ticket_info
            .get(&drop_id)
            .expect("No ticket info found for drop")
            .price;
        ...
        let max_markup = event.max_markup;
    ...
    pub fn get_events_per_funder(
        &self,
        account_id: AccountId,
        limit: Option<u64>,
        from_index: Option<u64>,
    ) -> Vec<ExtEventDetails> {
        let funder_events: Vec<EventDetails> = self
            .event_by_id
            .iter()
            .filter(|x| x.1.funder_id == account_id.clone())
            .map(|x| x.1)
            .collect();
    ...
    pub fn get_event_supply_for_funder(&self, account_id: AccountId) -> u64 {
        self.event_by_id
            .iter()
            .filter(|x| x.1.funder_id == account_id)
            .count() as u64
    }
    ...
    pub fn get_resales_per_drop(&self, drop_id: DropId) -> Vec<ResaleInfo> {
        ...
        self.resales
            .get(&drop_id)
            .unwrap_or(UnorderedMap::new(StorageKeys::ResalesPerDropInner {
                identifier_hash,
            }))
            .iter()
            .map(|x| x.1)
    ...
    pub fn get_resales_per_event(
        &self,
        event_id: EventID,
    ) -> Option<HashMap<DropId, Vec<ResaleInfo>>> {
        let event = self
            .event_by_id
            .get(&event_id)
    ...
    pub fn get_all_resales(&self) -> HashMap<EventID, HashMap<DropId, Vec<ResaleInfo>>> {
        ...
        for event_id in all_event_id {
            let resales = self
                .get_resales_per_event(event_id.clone())
                .expect("get_resales_per_event returning None somehow");
    ...
    pub fn get_ticket_price(&self, drop_id: DropId) -> U128 {
        ...
        self.event_by_id
            ...
            .ticket_info
            .get(&drop_id)
            .expect("No price found for drop")
            .price
            .clone()
    }
